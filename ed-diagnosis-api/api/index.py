...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        """Handles GET requests by returning a basic API status message."""
        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
        response = {"status": "success", "message": "ED Diagnosis API is running"}
        self.wfile.write(json.dumps(response).encode('utf-8'))

    def do_POST(self):
        """Handles POST requests for ED diagnosis."""
        try:
